...
> **セルフサービス登録（推奨・手動SQL不要）**：`schema.sql` 適用後は、利用者が本番URLで
> **新規登録 → ログイン → 「会社を新しく作る」** を選ぶだけで、自分の会社が作られ管理者になります
> （サーバー側 RPC `create_my_company` が安全に処理）。家族・職人は管理者が「共有」タブに表示される
> **招待コード**を伝え、新規登録後に「招待コードで参加」すれば同じ会社に入れます。
> 以下の手動SQLは、運用者がまとめて用意したい場合の代替手段です。
...
- レコードIDはクライアントで **UUID(v4)** を採番し、Postgres の `uuid` 主キーにそのまま保存（型不一致なし）
- デモ用サンプルデータは**デモ(端末内保存)モードのみ**投入。本番では会社の実データだけを扱う
